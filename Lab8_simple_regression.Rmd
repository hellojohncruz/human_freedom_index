...
There are 1,458 observations with 123 variables.
...
Utilizing a scatterplot, we can see somewhat of a step pattern increase along the *pf_expression_control* variable. However, overall it does look to have a linear relationship and using a linear model to predict the personal freedom score may be viable.
...
We can see from the scatterplot a moderate, positive increase where as pf_expression_control increases, the pf_score increases. However, the trend is not entirely linear as the tail ends when pf_expression_control is ~ <2.5 and greater than 7.5, outliers begin to form a small concave down parabola.  
...
The smallest sum of squares I got was 971.172. Another classmate got 1033. 
...
As pf_expression_control increases by one unit, human_freedom increases by 0.349862. 
    
```{r lm-model-hf-score}
lm_mod <- lm(hf_score ~ pf_expression_control, data = hfi)
...
We can fined where 6.7 lands on the x-axis (pf_expression_control) and move up to where the regression line meets; roughly a 7.8 pf_score. However, lets be more accurate using the predict function and our regression equation which tells us it is 7.49776. 

```{r predict}
test_data <- data.frame(pf_expression_control = 6.7)

predict(lm_mod, test_data)
```

Given that the dataset does not contain values for pf_expression_control = 6.7, as it has 0.25 increments, I will collect the next closest data points which is 6.75. Let's now determine the residuals for each one and calculate the average.  
...
resid_diff <-
  original_pts$pf_score - predict(lm_mod, test_data)

mean(resid_diff)
```

The residuals tell us that we will either overestimate or underestimate given that the linear regression model is trying to reduce the sum of sqaured errors. However, looking at the average of the residuals, we will be generally underestimating what 6.7 should be. 
...
Overall there does not seem to be any unusual pattern as the values are scattered around zero. This means that in general, there is a linear relationship between both variables. 
...
Yes the normal condition appears to be met with some slight variation in the tails. 
...
Yes, the constant variability condition is met as there is not a specific pattern being seen.
...
ef_trade_tariffs vs. ef_score

There does seem to be a linear relationship, however quite weak. 
    
```{r scatterplot-2}
hfi |> 
  ggplot(aes(x = ef_trade_tariffs, y = ef_score)) +
  geom_point()
```
...
```{r lm-model-ef-score}
lm_mod_2 <- lm(ef_score ~ ef_trade_tariffs, data = hfi)

summary(lm_mod_2)
```

The linear model between ef_trade_tariffs vs. ef_score is significantly weaker with an $R^2$ of 0.2292 compared to `pf_expression_control` and `pf_score` and an $R^2$ of 0.6342
...
```{r scatterplot-3}
hfi |> 
  ggplot(aes(x = pf_religion, y = pf_score)) +
  geom_point()
```
    
```{r lm-model-hf-score-2}
lm_mod_2 <- lm(pf_score ~ pf_religion, data = hfi)

summary(lm_mod_2)
```

Looking into the relationship between pf_score vs. pf_religion I would have thought it would have a stronger association then it shows. This may be due to the skew viewpoints living in the United States and how religion is portrayed within media. Overall throughout the globe, religion does not seem to be a major point between personal freedoms of religion to their overall personal freedom score. 
